     ...
     .               dx,dy,tol,isf,itstart,pi,amplit,prob,tpulse
      ...
      double precision dke(Nx,Ny)
      common /param/ gamma01,ro01,Diffgamma,dke0,dk1


      call function1(t,Nx,Ny,gamma,ro,f1,f2,Phi,dke)
      ...
      do i=1,Nx
       ...
       if (i .gt. 100) then
        ...
        if (t .ge. tE) then
            gammaprime(i,j)=1.d0/depsilon*(s1*s2*Phi(i,j)
     .            -dke(i,j)*gamma(i,j)/dke0)+depsilon*gLaplace(i,j)-
...
!     ***********************************************************
      subroutine function1(t,Nx,Ny,gamma,ro,f1,f2,Phi,dke)
...
!        dke(i,j)=dke0+4.5*
!     .      (1-(1.d0-(j-1-(Ny-1.d0)/2)**2/((Ny-1.d0)/2)**2))
       enddo
      enddo
      ...
      if(Ny .eq. 80)then
        do i=1,Nx
            dke(i,1)=dke0+ke0Dif*(1-0.0000)
            dke(i,2)=dke0+ke0Dif*(1-0.7946)
            dke(i,3)=dke0+ke0Dif*(1-0.9720)
            dke(i,4)=dke0+ke0Dif*(1-0.9976)
            dke(i,5)=dke0+ke0Dif*(1-0.9999)

            dke(i,76)=dke0+ke0Dif*(1-0.9999)
            dke(i,77)=dke0+ke0Dif*(1-0.9976)
            dke(i,78)=dke0+ke0Dif*(1-0.9720)
            dke(i,79)=dke0+ke0Dif*(1-0.7946)
            dke(i,80)=dke0+ke0Dif*(1-0.0000)
      ...
      elseif(Ny .eq. 120)then
        do i=1,Nx
            dke(i,1)=dke0+ke0Dif*(1-0.0000)
            dke(i,2)=dke0+ke0Dif*(1-0.6350)
            dke(i,3)=dke0+ke0Dif*(1-0.8903)
            dke(i,4)=dke0+ke0Dif*(1-0.9708)
            dke(i,5)=dke0+ke0Dif*(1-0.9943)
            dke(i,6)=dke0+ke0Dif*(1-0.9991)
            dke(i,7)=dke0+ke0Dif*(1-0.9999)

            dke(i,114)=dke0+ke0Dif*(1-0.9999)
            dke(i,115)=dke0+ke0Dif*(1-0.9991)
            dke(i,116)=dke0+ke0Dif*(1-0.9943)
            dke(i,117)=dke0+ke0Dif*(1-0.9708)
            dke(i,118)=dke0+ke0Dif*(1-0.8903)
            dke(i,119)=dke0+ke0Dif*(1-0.6350)
            dke(i,120)=dke0+ke0Dif*(1-0.0000)
